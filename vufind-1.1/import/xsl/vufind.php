...
class VuFind
{
    ...
    {
        include_once 'services/MyResearch/lib/Change_tracker.php';
        $date = strtotime($date);
        $tracker = new Change_tracker();
        $tracker->index($core, $id, $date);
        $iso8601 = 'Y-m-d\TH:i:s\Z';
        return date($iso8601, strtotime($tracker->first_indexed));
    }
    ...
    public static function getLastIndexed($core, $id, $date)
    {
        ...
        $tracker->index($core, $id, $date);
        $iso8601 = 'Y-m-d\TH:i:s\Z';
        return date($iso8601, strtotime($tracker->last_indexed));
    }
    ...
    public static function harvestWithAperture($url)
    {
        ...
        $settings = getExtraConfigArray('fulltext');
    ...
    {
        static $articles = array('a', 'an', 'the', 'de', 'het', 'een', 'le', 'la', 'die', 'der', 'des', 'das');
    ...
    /**
     * Read the first letter from the title, ignoring any articles.
     *
     * @static
     * @param  $in
     * @return void
     */
    public static function getLetter($in)
    {
        static $articles = array('a', 'an', 'the', 'de', 'het', 'een', 'le', 'la', 'die', 'der', 'des', 'das');

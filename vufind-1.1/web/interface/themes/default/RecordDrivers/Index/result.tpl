<div id="record{$summId|escape}" class="yui-ge">
  <div class="yui-u first">
    ...
    <div class="resultitem">
      ...
      <a href="{$url}/Record/{$summId|escape:"url"}{$summBookmark}" class="title">{if !empty($summHighlightedTitle)}{$summHighlightedTitle|regex_replace:"/\s.\Z/":""|addEllipsis:$summTitle|highlight}{elseif !$summTitle}{translate text='Title not available'}{else}{$summTitle|truncate:180:"..."|escape}{/if}{$summExtendedTitle}</a>
      </div>
      ...
      <b>{translate text='Located'}:</b> <span id="location{$summId|escape}">{translate text='Loading'}</span>
          {elseif !empty($summCallNo)}
           <b>{translate text='Collector'}:</b> {$summCallNo|escape}
          {/if}
      {*{elseif !empty($summCallNo)}

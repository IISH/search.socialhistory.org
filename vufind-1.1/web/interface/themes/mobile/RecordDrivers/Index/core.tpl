...
{* Display Title *}
<span class="graytitle">{$coreShortTitle|escape}</span>
{if $coreSubtitle}<span class="graytitle">{$coreSubtitle|escape}</span>{/if}
{if $coreTitleSection}<span class="graytitle">{$coreTitleSection|escape}</span>{/if}
{if $coreTitleStatement}<span class="graytitle">{$coreTitleStatement|escape}</span>{/if}
{* End Title *}
...
<table>
  ...
  <tr valign="top">
    ...
    <td>
      {* TODO: implement "add tag" in mobile theme

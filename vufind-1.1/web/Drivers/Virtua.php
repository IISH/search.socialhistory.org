...
class Virtua implements DriverInterface
{
    ...
    function __construct()
    {
        ...
        $this->_config = parse_ini_file(
            dirname(__FILE__) . '/../conf/Virtua.ini', true
        );
     ...
     * @access private
     */
    private function _checkHoldAllowed($patron_id, $holdings)
    {
        // Get the patron type
    ...
    private function _renderSubPattern($data)
    {
        ...
        foreach ($data['pattern'] as $p) {
            ...
            if ($is_ch_pattern === false) {
                $return_string .= $this->_renderPartSubPattern($sub_pattern);
                $sub_pattern = array();
            }
    ...
    private function _renderPattern($patterns, $field)
    {
        ...
        if (isset($field['pattern']) && isset($patterns[$field['pattern']])) {
            ...
            foreach ($field['data'] as $d) {
                ...
                $p = $this->_getField($pattern, $d['code']);
                ...
                $return['en_ch'] = $this->_renderSubPattern($this_en_ch);
                $return['other'] = $this->_renderOtherPattern($this_other);
            }
        } else {
            // Otherwise just return the a subfield as a note
            $return['other']['notes'][] = $this->_getField($field['data'], 'a');
        }
        return $return;
    }
     ...
     * @access private
     */
    private function _renderSerialHoldings($holdings_marc)
    {
        ...
        foreach ($holdings_data as $row) {
            $rendered_list[] = $this->_renderPattern($patterns, $row);
        }
